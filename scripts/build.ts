...
export default async function(
  ...
  for (const packageName of sortedPackages) {
    ...
    const resources = files
      ...
      .filter(fileName => {
        ...
        if (/helpers-amp\\temp/.test(fileName)) {
          return false;
        }
        // Schematics template files.
        if (pkgJson['schematics'] &&
         (fileName.match(/(\/|\\)files(\/|\\)/) || fileName.match(/(\/|\\)\w+-files(\/|\\)/)) || fileName.match(/files(\/|\\)/)) {
          return true;
        }
        ...
        if (fileName.indexOf('helpers-amp/temp') !== -1) {
          return false;
        }
  ...
  // logger.info('Copying extra Schematics Utilities resources...');
  // const schematicsPackages = ['@ampgular/amp', '@ampgular/seo'];
  // for (const packageName of schematicsPackages) {
  //   const pkg = packages[packageName];
  //   const pathSplit = pkg.build.split('/')
  //   const ParentPath = pathSplit.slice(0,pathSplit.length-1).join("/")
  //   const parentPath = path.normalize(ParentPath)
  //   //const files = glob.sync(path.join('../',pkg.root, '**/*'), { dot: true, nodir: true });


  //   _recursiveCopy(path.join(parentPath, 'utility'), path.join(pkg.dist, 'utility'),logger);
  // }
